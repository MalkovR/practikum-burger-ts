...
import {
  getWsOrders,
  getWsOrdersTotal,
  getWsOrdersTotalToday,
} from "../../services/feed-ws/selectors.ts";
  ...
  return (
    <section className={style.orders_info_container}>
      ...
      <div className={style.total}>
        ...
        <span className="text text_type_digits-large">{ordersTotal}</span>
      </div>
      <div className={style.total}>
        <span className="text text_type_main-medium">
          Выполнено за сегодня:
        </span>
        <span className="text text_type_digits-large">{ordersTotalToday}</span>
      </div>
    </section>
  );
};

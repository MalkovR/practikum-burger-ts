...
export const OrderItem = ({ order, isVisible }: TOrderItem) => {
  ...
  const totalOrder = order.ingredients.reduce((sum, id) => {
    const ingredient = ingredients.find(
      (item: TBurgerIngredient) => id === item._id,
    );
  ...
  return (
    <div className={style.order_item_container}>
      ...
      <h3 className={`${style.text_left} text text_type_main-medium`}>
        {order.name}
      </h3>
      {isVisible && (
        <p
          className={`text text_type_main-default ${order.status === "done" ? style.order_done : style.text_left}`}
        >

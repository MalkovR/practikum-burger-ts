...
export const OrderInfo = () => {
    ...
    for (const id of selectedOrder.ingredients) {
        uniqueOrderIngredients[id] =
            (uniqueOrderIngredients[id] ?? 0) + 1;
    }

    const totalOrder = selectedOrder.ingredients.reduce((sum, id) => {
        return sum + (allIngredients.find((item: TBurgerIngredient) => id === item._id)?.price ?? 0);
    }, 0);
    ...
    return (
        <div className={style.order_info_container}>
            ...
            <div className={style.order_name}>
                ...
                <p className={`text text_type_main-default ${selectedOrder?.status === 'done' ? style.order_done : ''}`}>
                        ...
                        return (
                            <div className={style.order_ingredient} key={ingredient?._id}>
                                <div className={style.left_part}>
                                    ...
                                    <p className={`${style.ingredient_name} text text_type_main-default ml-4`}>
                                        {ingredient?.name}
                                    </p>
                                </div>
            ...
            <div className={`${style.price} mt-10`}>
                <p className={`${style.timestamp} text text_type_main-default text_color_inactive`}>
                    {selectedOrder?.createdAt && <FormattedDate date={new Date(selectedOrder?.createdAt)} />}
                </p>
